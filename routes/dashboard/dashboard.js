...
router.get('/V00/dashboard', function(req, res, next) {
    ...
    }else if(tsec === '2345678' && req.query.$filter === 'productType==TT')
    ...
    else if (req.query.$filter === 'productType==SI,idContract==SIMXP0000003')
        return res.json(dashboard_04_SI);
    else if (req.query.$filter === 'productType==SI,idContract==SIMXP0000005')
        return res.json(dashboard_04_SI);
    ...
    else if ((tsec != undefined || tsec != '') && req.query.$filter === 'productType==TT')
        return res.json(dashboard_01);
    else if ((tsec == undefined || tsec == '12345678') && req.query.$filter === 'productType==IN,idContract==INMXP0000001,currency==MXP')
        return res.json(dashboard_03_IN); 
    else if ((tsec == undefined || tsec == '12345678') && req.query.$filter === 'productType==IN,idContract==INMXP0000002,currency==MXP')
        return res.json(dashboard_error_02);
    else if ((tsec == undefined || tsec == '12345678') && req.query.$filter === 'productType==IN,idContract==INMXP0000003,currency==MXP')
        return res.json(dashboard_04_IN);
    else if ((tsec == undefined || tsec == '') && req.query.$filter === 'productType==IN,idContract==CAUSD0000001,currency==USD')
